...
filepath byte "C:\Users\Islam\Desktop\University\Assembly\Project\Project_Template\TestCase2.txt",0
savefilepath byte "C:\Users\Islam\Desktop\University\Assembly\Project\Project_Template\output.txt",0
filehandle byte ?
datarange =1000
...
;Returns: File contains the indecies of the least motifs found. 
;-------------------------------------------------------------------------------------------------------------------------------------
 Display proc 
...
Highlight proc 
	...
	mov ebx , 0
	mov ecx , pattern
	; Initializing the original motif with a different color
	InitOriginalMotif:
		mov esi , startindex
		add esi , ebx
		mov dnacolors[esi] , 2
	...
	;Loop on motifs, put mutations with red and the others with green.
	IterateOnMotifs:
	mov eax , prevoutput[0]
	mov startindex , eax
	mov edx , 0
	...
	mov searchindex , eax
		...
		CompareMotifs:
			...
			cmp ah , al
			JE ColorGreen
			mov dnacolors[esi] , 1 ; 1 = red color
			jmp done
			ColorGreen:
			mov dnacolors[esi] , 2 ; 2 =green color , 0 =white color by default 
			done:
			inc edx
		loop CompareMotifs
		pop ecx
